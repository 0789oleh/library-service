...
async def test_authenticate_member_success(db_session: Session):
    ...
    authenticated_member = authenticate_member(db_session, "test@example.com", "testpassword")
...
async def test_authenticate_member_invalid(db_session: Session):
    ...
    authenticated_member = authenticate_member(db_session, "test@example.com", "wrongpassword")
    assert authenticated_member is None

    authenticated_member = authenticate_member(db_session, "nonexistent@example.com", "testpassword")
    assert authenticated_member is None
...
async def test_login_member(db_session: Session):
    ...
    token_response = login_member(login_request, db_session)
...
async def test_login_member_invalid(db_session: Session):
    ...
    with pytest.raises(HTTPException) as exc:
        login_member(login_request, db_session)
